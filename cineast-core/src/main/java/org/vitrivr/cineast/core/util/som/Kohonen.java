...
public class Kohonen extends JFrame
{
	...
	public void readFile(File input)
	{
		...
		/*for (int i = 0; i < xVal*yVal; i++) {
			if (training.nearestNodeAssignment[i] != 0) {
				//http://10.34.58.178/V3C1/V3C1jpg/v_01641/v_01641_256.jpg
				String id = ids.get(training.nearestNodeAssignment[i]);
				System.out.println("http://10.34.58.178/V3C1/V3C1jpg/v"+id.substring(0, id.lastIndexOf("_"))+"/v"+id+".jpg");
			}
		}*/
		plot(xVal, yVal, training);
	}
	 ...
	 * 
	 * */
	 public void plot(int xDim, int yDim, SOM som)
	 {
		 ...
		 int index;
		 int columnCount = 0;
		 int[] nearestEntryOfNode = som.nearestEntryOfNode;
		 for(int i = 0; i < counts.length; i++)
		 {
			 ...
			 if (nearestEntryOfNode[index] != -1) {
				 String id = ids.get(nearestEntryOfNode[index]);
				 //System.out.println("http://10.34.58.178/V3C1/V3C1jpg/v"+id.substring(0, id.lastIndexOf("_"))+"/v"+id+".jpg");
				 if (counts[index] != 0) {
					 //System.out.println("i: " + index + " counts: " + counts[index] + " link: http://10.34.58.178/V3C1/V3C1jpg/v" + id.substring(0, id.lastIndexOf("_")) + "/v" + id + ".jpg");
				 } else {
					 System.out.println("INFO: count yes but no nearest!!!");
				 }
			 }
			 //jB[index].setForeground(new Color(r, g, b));
			 //jB[index].setBorderPainted(false);
		 ...
		 int average =  som.getDistances().length / counts.length;
